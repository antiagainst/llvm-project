// RUN: mlir-opt -split-input-file -test-linalg-transform-patterns=test-split-padding-patterns %s | FileCheck %s

// CHECK-LABEL: func @pad_all_zero_sizes
func @pad_all_zero_sizes(%input: tensor<?x?x?xf32>) -> tensor<?x?x?xf32> {
  %f0 = arith.constant 0.0 : f32
  %c0 = arith.constant 0 : index
  %0 = linalg.pad_tensor %input low[0, %c0, 0] high[%c0, 0, 0] {
...
func @pad_non_zero_sizes(%input: tensor<?x?x8xf32>, %low0: index, %high1: index) -> tensor<?x?x8xf32> {
  %f0 = arith.constant 0.0 : f32
  %0 = linalg.pad_tensor %input low[%low0, 0, 0] high[0, %high1, 0] {
  ^bb0(%dim0: index, %dim1: index, %dim2: index):
    linalg.yield %f0 : f32
...
// CHECK: } else {
// CHECK:   %[[PAD:.+]] = linalg.pad_tensor %[[INPUT]] low[%[[LOW0]], 0, 0] high[0, %[[HIGH1]], 0]  {
// CHECK:   ^bb0(%{{.+}}: index, %{{.+}}: index, %{{.+}}: index):
// CHECK:     linalg.yield %[[F0]] : f32

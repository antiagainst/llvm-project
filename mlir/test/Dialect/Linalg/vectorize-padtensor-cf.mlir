// RUN: mlir-opt -split-input-file -mlir-print-local-scope -test-linalg-transform-patterns=test-vectorize-padtensor-with-cf -canonicalize -cse %s | FileCheck %s

func @pad_tensor(%source: tensor<1x?x?x3xf32>, %low1: index, %low2: index, %high1: index, %high2: index) -> tensor<1x2x2x3xf32> {
  %cst = arith.constant 0.0 : f32
  %pad = linalg.pad_tensor %source low[0, %low1, %low2, 0] high[0, %high1, %high2, 0]  {

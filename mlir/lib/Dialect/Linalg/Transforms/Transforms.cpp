...
namespace {
/// Concretizes linalg.pad_tensor op's result shape if its source op implements
/// OffsetSizeAndStrideOpInterface. For example, pad_tensor(extract_slice).
struct ConcretizePadResultShape final
...
void linalg::populateConcretizePadTensorResultShapePatterns(
    RewritePatternSet &patterns, PatternBenefit benefit) {
  patterns.add<ConcretizePadResultShape>(patterns.getContext(), benefit);
}

...
namespace {
/// Vectorizes linalg.pad_tensor ops by generating scf.if guards around
/// vector.transfer_read ops, e.g., converting the following IR:
///
/// ```
/// %pad = linalg.pad_tensor %s ... : tensor<1x?x?x3xf32> -> tensor<1x2x2x3xf32>
/// ```
///
/// into
///
...
/// ```
struct VectorizePadTensorWithIfOps final
...
void mlir::linalg::populateVectorizePadTensorOpWithControlFlowPatterns(
    RewritePatternSet &patterns, PatternBenefit baseBenefit) {
  patterns.add<VectorizePadTensorWithIfOps>(patterns.getContext(), baseBenefit);
}
